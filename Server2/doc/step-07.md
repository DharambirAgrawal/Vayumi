...
- Sub-agents, signal bus, task board, notifier (Step 08+).
- MCP adapter and capability bundles (Steps 09 and 17).
- LanceDB retrieval and summarizer (Steps 11–12).
- File uploads / attachments (Step 16).

---

## Risks and how we'll catch them

- Tavily/DDG fallback flakiness — mock httpx in unit tests; enforce timeouts.
...
- Step 08 adds sub-agent worker + signal bus; tool runner should be reusable without changes.
- Step 09 will expose `web_search`/`memory_recall` to sub-agent capabilities.
