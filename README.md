...
- **Layered memory**
  - Versioned key-value facts (`facts` table) with full supersession history — updating a fact keeps the old value queryable rather than overwriting it.
...
- Meeting *sync*: the client records and analyzes meetings on-device and uploads the resulting summary/key points/action items/transcript — audio itself never leaves the device.
...
1. **Transport** (`transport/ws.py`) — a single WebSocket endpoint per user (session singleton: a second connection cleanly supersedes the first), speaking a typed JSON + binary-PCM protocol.
...
3. **Engine** (`engine/pool.py`) — a single `llama-server` process shared across the whole system through a priority queue: the live conversation (P0) always preempts sub-agent work (P1) and the background summarizer (P2).
4. **Orchestrator** (`orchestrator/supervisor.py`) — runs one "turn" at a time: builds the prompt (warm profile + recent/compressed history + retrieved memory), gets a completion, executes any native tool calls, parses fallback directives (`[REMEMBER]`, `[RECALL]`, `[DELEGATE]`, `[ANSWER_TO]`, `[STOP_TASK]`), decides whether the reply goes out as voice, chat, or both (`respond_via`), and streams the result back over the transport.
